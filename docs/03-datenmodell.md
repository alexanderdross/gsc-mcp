...
> **Maßgebliche Quelle:** Das lauffähige DDL liegt in `packages/db/migrations/0001_init.sql` und wird von der CI (`scripts/validate-ddl.mjs`) gegen ein echtes PostgreSQL geprüft. Dieses Dokument erklärt die Entscheidungen dahinter; bei Abweichungen gilt die Migration. Getippter Zugriff erfolgt über die Drizzle-Modelle in `packages/db/src/schema.ts`, deren Übereinstimmung mit der Migration ein Test absichert.
