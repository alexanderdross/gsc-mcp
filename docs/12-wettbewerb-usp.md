...
**1 — Vollständig statt Stichprobe — ab dem Tag der Verbindung.** Alle anderen speichern die Top-N der API. Wir arbeiten auf dem Bulk Export, der alles enthält außer den von Google anonymisierten Anfragen. Wie groß der Unterschied ist, ist an eigenen Daten gemessen: Bei `sc-domain:aip.aero` entfallen auf die 100 klickstärksten Suchanfragen **8,3 % der Klicks** — rund 92 % des Geschehens liegen darunter. Das ist keine Marketingaussage, sondern eine nachrechenbare Zahl.

**Die Ehrlichkeitsgrenze dieser Aussage muss mitgesprochen werden**, sonst fällt sie beim ersten kritischen Kunden in sich zusammen: *Vollständig* ist nur, was ab dem Tag der Export-Aktivierung eingeht. Die bis zu 16 Monate Historie davor stammen aus dem API-Backfill und sind damit selbst eine Top-N-Stichprobe — dieselbe Grenze, die für alle gilt. Der ehrliche Satz lautet deshalb nicht „vollständige Historie", sondern: **„vollständig ab heute, plus bis zu 16 Monate gesampelte Historie dahinter."** Das ist immer noch eine Alleinstellung — *vollständig ab heute* kann im ganzen Feld niemand, weil alle anderen dauerhaft an der API hängen. Aber die beiden Wörter „vollständig" und „Historie" dürfen im Marketing nie im selben Satz stehen, ohne dass die Naht zwischen Backfill und Export benannt wird.
