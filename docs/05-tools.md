...
Alle datentragenden Tools hängen an injizierbaren Repo-Schnittstellen (`WarehouseRepo` bzw. `IndexingRepo`) — die Handler-Logik ist damit ohne Datenbank getestet. Beide konkreten Implementierungen sind gegen ein echtes PostgreSQL getestet: `WarehouseRepository` (reine Drizzle-Abfragen) liegt in `packages/db`; `IndexingRepository` ist ein zusammengesetzter Adapter aus GSC-Client (`packages/gsc-client`), Inspektions-Cache und Tagesbudget (`packages/db`) und einer injizierten Queue, und liegt am Kompositionspunkt in `apps/app`. `get_google_updates` (I/O-frei) und `export_data` (über eine injizierte `ExportStore`-Schnittstelle) sind ebenfalls implementiert.

Das MCP-Transport-Gerüst steht in `apps/app/src/mcp/` (netzunabhängig, testbar): JSON-RPC-2.0-Umschlag (`jsonrpc.ts`), Zod→JSON-Schema für `inputSchema` (`schema.ts`), das Methodenrouting `initialize`/`tools/list`/`tools/call` über Registry und Router (`dispatch.ts`), der `Mcp-Session-Id`-Sitzungsspeicher (`session.ts`) und der JSON-Antwortpfad des `/mcp`-Endpunkts (`transport.ts`). Offen bleibt die netzgebundene Verdrahtung: der laufende HTTP-Server mit dem SSE-Strom samt 30-s-Keepalive ([docs/01]), OAuth als Authentifikator sowie die laufende Queue.
