...
Alle datentragenden Tools hängen an injizierbaren Repo-Schnittstellen (`WarehouseRepo` bzw. `IndexingRepo`) — die Handler-Logik ist damit ohne Datenbank getestet. Der `WarehouseRepo`-Port und seine konkrete Implementierung (`WarehouseRepository`, Drizzle-Abfragen) liegen in `packages/db` und sind gegen ein echtes PostgreSQL getestet. `get_google_updates` (I/O-frei, gepflegter Katalog) und `export_data` (über eine injizierte `ExportStore`-Schnittstelle) sind implementiert. Offen bleiben nur noch die `IndexingRepo`-Implementierung gegen `packages/gsc-client` sowie der MCP-Transport.
