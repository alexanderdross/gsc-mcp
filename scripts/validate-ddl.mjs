...
const psql = (sql, file) =>
  execFileSync(
    "psql",
    [PGURL, "-v", "ON_ERROR_STOP=1", "-tAq", ...(file ? ["-f", file] : ["-c", sql])],
    { encoding: "utf8" },
  );
...
try {
  ...
  ok(
    `Migration ${MIGRATION} fehlerfrei aufgebaut (${schema.split("CREATE ").length - 1} CREATE-Anweisungen)`,
  );
} catch (e) {
...
const checks = [
  ...
  [
    "Tabellen in wh",
    "select count(*) from information_schema.tables where table_schema='wh'",
    (n) => n >= 12,
  ],
  [
    "partitionierte Faktentabellen",
    "select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace where c.relkind='p' and n.nspname='wh'",
    (n) => n === 6,
  ],
  [
    "Partitionsfunktion",
    "select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='wh' and p.proname='ensure_month_partitions'",
    (n) => n === 1,
  ],
];
...
const parts = psql(`select tableoid::regclass::text from wh.fact_totals order by day`)
  .trim()
  .split("\n");
...
const pos = psql(
  `select round((sum(position_sum)/sum(impressions))::numeric,2) from wh.fact_totals`,
).trim();
pos === "7.62"
