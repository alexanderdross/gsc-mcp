...
const body =
  '<div class="wrap">' + src.split('<div class="wrap">').slice(1).join('<div class="wrap">');
writeFileSync(
  ".render/preview.html",
...
for (const [scheme, width] of [
  ...
  const overflow = await page.evaluate(
    (w) => (document.documentElement.scrollWidth > w ? document.documentElement.scrollWidth : 0),
    width,
  );
  overflow
    ? fail(`${scheme} @${width}px: seitlicher Überlauf auf ${overflow}px`)
    : ok(`${scheme} @${width}px: kein seitlicher Überlauf`);
  ...
  contrast.transparent && fail(`${scheme}: body ohne eigene Hintergrundfarbe`);
  contrast.same
    ? fail(`${scheme}: Überschrift hat dieselbe Farbe wie der Grund`)
    : ok(`${scheme}: Vorder- und Hintergrund unterscheiden sich`);
