...
for (const f of readdirSync("docs").filter((f) => f.endsWith(".md"))) {
  readme.includes(`docs/${f}`) || fail(`docs/${f} ist im README nicht verlinkt`);
}
...
const comparative =
  /Cloudflare-(Variante|Fassung)|D1-Fassung|Vorversion|urspr[üu]nglich|revidiert|Vorgängerfassung/i;
for (const file of files) {
  for (const [line, i] of readFileSync(file, "utf8")
    .split("\n")
    .map((l, i) => [l, i + 1])) {
    for (const [re, label] of forbidden) {
      if (re.test(line) && !comparative.test(line))
        fail(`${file}:${i}: Rückstand „${label}" ohne Vergleichskontext`);
    }
  }
}
...
const htmlChecks = [
  ...
  [
    !/<!DOCTYPE|<html[\s>]|<body[\s>]/i.test(html),
    "kein eigenes doctype/html/body — wird beim Publish umschlossen",
  ],
  [/:root\s*{[^}]*--paper/.test(html), "helle Palette auf bare :root definiert"],
  [
    /prefers-color-scheme:\s*dark/.test(html) && /\[data-theme="dark"\]/.test(html),
    "beide Dunkelmodus-Pfade abgedeckt",
  ],
  [
    /body\s*{[^}]*background:\s*var\(--paper\)/.test(html),
    "body setzt eine eigene Hintergrundfarbe",
  ],
